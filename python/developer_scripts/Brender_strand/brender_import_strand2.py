	...
	def execute (self, context):
		...
		for file in files:
			...
			for strand in range(1,len(xns)):
				...
				# Display on frame so our animation starts on frame 1 not 0
				# Don't show it before this frame
				cmo.hide = cmo.hide_render = True
				cmo.keyframe_insert(data_path='hide', frame = frame-1)
				cmo.keyframe_insert(data_path='hide_render', frame = frame-1)

				# # Only show it this frame
				cmo.hide = cmo.hide_render = False
				cmo.keyframe_insert(data_path='hide', frame = frame)
				cmo.keyframe_insert(data_path='hide_render', frame = frame)
